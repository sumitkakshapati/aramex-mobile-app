...
class SaveWalletCubit extends Cubit<CommonState> {
  final AccountRepository accountRepository;
  SaveWalletCubit({required this.accountRepository})
      : super(CommonInitialState());

  saveWallet({required String username, required int walletId}) async {
    emit(CommonLoadingState());
    final _res = await accountRepository.saveWallet(
        walletId: walletId, username: username);
    if (_res.status == Status.Success && _res.data != null) {
      emit(CommonDataSuccessState<UserWallet>(data: _res.data));
    } else {
      emit(CommonErrorState(message: _res.message ?? "Unable to save wallet"));
    }
  }
}
